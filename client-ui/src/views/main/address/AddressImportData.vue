    ...
    <el-form :model="form" :rules="rules" ref="ruleFormRef">
      ...
      <div class="form_items">
        ...
        >
          <el-select
            v-model="form[field]"
            :data-test="`addressImportData_${field}`"
          >
            <el-option
      ...
      <div class="d-flex justify-content-end">
        <el-button
          data-test="addressImportDataFormCancelButton"
          @click="resetForm(ruleFormRef)"
          >Clear</el-button
        >
        <el-button
          data-test="addressImportDataFormSubmitButton"
          type="primary"
...
const { error } = storeToRefs(useAddressStore());
const { createManyAddress } = useAddressStore();
...
const addressInsertData: any = ref([]);
...
const handelUploadFile: UploadProps["onChange"] = (rawFile: any) => {
  if (rawFile) {
    ...
    reader.onload = (e) => {
      ...
      addressInsertData.value = jsonData;
    };
...
const onSubmit = (formEl: InstanceType<typeof ElForm> | undefined) => {
  ...
  formEl.validate(async (valid) => {
    ...
    if (valid && !isEqual(initialState, rest)) {
      addressInsertData.value = addressInsertData.value.map((item: any) => {
        let res: any = {};
        for (let key in rest) {
      ...
      await createManyAddress(addressInsertData.value);
