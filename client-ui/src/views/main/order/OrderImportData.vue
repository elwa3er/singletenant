    ...
    <el-form :model="form" :rules="rules" ref="ruleFormRef">
      ...
      <div class="form_items">
        ...
        >
          <el-select
            v-model="form[field]"
            :data-test="`orderImportData_${field}`"
          >
            <el-option
      ...
      <div class="d-flex justify-content-end">
        <el-button
          data-test="orderImportDataFormCancelButton"
          @click="resetForm(ruleFormRef)"
          >Clear</el-button
        >
        <el-button
          data-test="orderImportDataFormSubmitButton"
          type="primary"
...
const { error } = storeToRefs(useOrderStore());
const { createManyOrder } = useOrderStore();
...
const handelUploadFile: UploadProps["onChange"] = (rawFile: any) => {
  if (rawFile) {
    ...
    reader.onload = (e) => {
      ...
      orderInsertData.value = jsonData;
    };
...
const onSubmit = (formEl: InstanceType<typeof ElForm> | undefined) => {
  ...
  formEl.validate(async (valid) => {
    ...
    if (valid && !isEqual(initialState, rest)) {
      orderInsertData.value = orderInsertData.value.map((item: any) => {
        let res: any = {};
        for (let key in rest) {
      ...
      await createManyOrder(orderInsertData.value);
