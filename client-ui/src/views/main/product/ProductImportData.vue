    ...
    <el-form :model="form" :rules="rules" ref="ruleFormRef">
      ...
      <div class="form_items">
        ...
        >
          <el-select
            v-model="form[field]"
            :data-test="`productImportData_${field}`"
          >
            <el-option
      ...
      <div class="d-flex justify-content-end">
        <el-button
          data-test="productImportDataFormCancelButton"
          @click="resetForm(ruleFormRef)"
          >Clear</el-button
        >
        <el-button
          data-test="productImportDataFormSubmitButton"
          type="primary"
...
const { error } = storeToRefs(useProductStore());
const { createManyProduct } = useProductStore();
const router = useRouter();

const initialState = { name: "", itemPrice: "", description: "" };
...
const productInsertData: any = ref([]);
...
const handelUploadFile: UploadProps["onChange"] = (rawFile: any) => {
  if (rawFile) {
    ...
    reader.onload = (e) => {
      ...
      productInsertData.value = jsonData;
    };
...
const onSubmit = (formEl: InstanceType<typeof ElForm> | undefined) => {
  ...
  formEl.validate(async (valid) => {
    ...
    if (valid && !isEqual(initialState, rest)) {
      productInsertData.value = productInsertData.value.map((item: any) => {
        let res: any = {};
        for (let key in rest) {
      ...
      await createManyProduct(productInsertData.value);
