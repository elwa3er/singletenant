...
async function dropTables(
  ...
  for (const table of tables) {
    await prisma.$executeRawUnsafe(`DROP TABLE public_test."${table}" CASCADE;`);
  }
}
...
async function getTables(prisma: PrismaClient): Promise<string[]> {
  const results: Array<{
    tablename: string;
  }> = await prisma.$queryRaw`SELECT tablename from pg_tables where schemaname = 'public_test';`;
