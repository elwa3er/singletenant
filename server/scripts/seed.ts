...
async function seed(bcryptSalt: Salt) {
  ...
  const queryGRANTUsageToAdmin = 'GRANT USAGE ON SCHEMA public_test TO "admin"';
  await client.$queryRawUnsafe(queryGRANTUsageToAdmin);

  const queryGRANTPRIVILEGESTABLESToAdmin =
    'GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public_test to "admin"';
  await client.$queryRawUnsafe(queryGRANTPRIVILEGESTABLESToAdmin);

  const queryGRANTPRIVILEGESSEQUENCESToAdmin =
    'GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public_test TO "admin"';
  ...
  const queryPolicySelect =
    ...
    "WHERE  policyname = 'public_test users are viewable by everyone.') THEN " +
    'create policy "public_test users are viewable by everyone." on public_test."User" for select using ( true );' +
    "END IF; " +
    "END " +
    "$do$; ";
  ...
  const queryPolicyInsert =
    ...
    'create policy "Users can insert their own users." on public_test."User" for insert with check ( auth_test.uid()::text = id );' +
    "END IF; " +
    "END " +
    "$do$; ";
  ...
  const queryPolicyUpdate =
    ...
    'create policy "Users can update own users or update invited not valid users" on public_test."User" for update using (((select invited_at from auth_test.users as au where au.id::text = "User".id and "User"."isValid" = false) is not null ) OR (auth_test.uid()::text = id));' +
    "END IF; " +
    "END " +
    "$do$; ";
  ...
  const queryPolicySelectId =
    ...
    'create policy "Users are viewable by users who created them." on public_test."User" for select using ( auth_test.uid()::text = id );' +
    "END IF; " +
    "END " +
    "$do$; ";
  ...
  const queryFunctionDeleteUser =
    "create or replace function public_test.handle_delete_user() " +
    "returns trigger " +
    "language plpgsql " +
    "security definer set search_path = public_test " +
    "as $$ " +
    "begin " +
    'delete from public_test."User" where id = old.id::text; ' +
    "return old; " +
    "end; " +
    "$$; ";
  ...
  const queryTriggerAddUser =
    ...
    "after insert or update on auth_test.users " +
    "for each row execute procedure public_test.handle_new_user() ";

  await client.$queryRawUnsafe(queryTriggerAddUser);

  const queryTriggerDeleteUser =
    "create or replace trigger on_auth_user_deleted " +
    "after delete on auth_test.users " +
    "for each row execute procedure public_test.handle_delete_user() ";
  ...
  await client.$queryRawUnsafe("GRANT USAGE ON SCHEMA public_test TO user_postgres");
  await client.$queryRawUnsafe(
    "GRANT ALL PRIVILEGES ON SCHEMA public_test TO user_postgres"
  );
  await client.$queryRawUnsafe(
    "GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public_test TO user_postgres"
  );
  await client.$queryRawUnsafe(
    "GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public_test TO user_postgres"
  );

  const queryGRANTSelectAuthUsersToUserPostgres =
    'GRANT SELECT ON auth_test.users TO "user_postgres"';
  ...
  await client.$queryRawUnsafe('GRANT USAGE ON SCHEMA public_test TO "user"');
  await client.$queryRawUnsafe(
    'GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public_test TO "user"'
  );
  ...
  */
  ...
  const queryFunctionIsValidUser =
    'create or replace function public_test.handle_valid_user()\n    returns trigger\n    language plpgsql\n    security definer set search_path = public_test\n    as $$ \n    begin\n      IF (new.password is not null and length(new.password) > 0) THEN\n        update public_test."User" set "isValid" = true where (id = new.id::text and ("isValid" is null or "isValid" = false ));\n      ELSE\n        update public_test."User" set "isValid" = false where (id = new.id::text and "isValid" is null);\n      END IF; \n      return new;\n    end;\n    $$;';
  await client.$queryRawUnsafe(queryFunctionIsValidUser);

  const queryTriggerValidUser =
    '\n    create or replace trigger handle_valid_user\n    after insert or update on public_test."User"\n    for each row execute procedure public_test.handle_valid_user()';
